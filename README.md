...
> 支持 14 种游戏垂类意图识别 + DeepSeek 大模型 + 联网实时搜索 + AI 流式总结。
...
│   ├── web-search.js             # 联网搜索模块（自建 Worker 代理 + 百度搜索 + AI 总结）
│   ├── response-builders.js      # 响应构建器（14 种意图的卡片生成 + 功能×游戏支持范围）
...
**核心特性**:
- 🏠 自建 Cloudflare Worker 搜索代理（稳定可控，免费 10 万次/天）
- 🔄 百度搜索直连降级（Worker 不可用时）
- 📦 搜索结果缓存（30 分钟 TTL）
- ⏱️ 15 秒总超时控制
- 📰 自动解析百度搜索结果为标题+摘要+链接
- 🤖 AI 流式总结（搜索成功后 DeepSeek 智能总结搜索要点）

**搜索架构**（三级降级）:
```
① 自建 Worker 代理（优先）→ ② 百度直连 → ③ 本地 Mock 数据
```

**降级策略**: Worker 失败 → 百度直连 → 搜索超时/失败 → 自动降级到本地 Mock 数据

**部署指南**: 参见 `search-worker/README-DEPLOY.md`

---

### 5. **deepseek-api.js** — 大模型 API 调用
**位置**: `js/deepseek-api.js`
**职责**: DeepSeek API 调用封装、请求构建、错误处理、AI 流式总结
...
- **流式总结能力**（`streamAISummary`）：攻略/资讯搜索结果 AI 智能总结 + SSE 流式输出 + 打字光标效果
...
| 修改联网搜索配置 | `js/web-search.js` 的 `WEB_SEARCH_CONFIG` + 部署 `search-worker/` |
...
- **联网搜索**: 自建 Cloudflare Worker 搜索代理 + 百度搜索（三级降级）
...
**Q: 联网搜索失败怎么办？**
A: 自动降级到本地 Mock 数据。如需稳定搜索，可部署 `search-worker/` 目录下的 Cloudflare Worker 代理（参见 `search-worker/README-DEPLOY.md`）。
...
- **v1.11** — 搜索引擎重构（自建 Cloudflare Worker 代理替代 CORS 竞速）+ AI 流式总结（攻略/资讯搜索结果 DeepSeek 总结 + 打字光标）+ 意图识别增强（纯英雄名搜索 + search_entry + @提醒）
